...
async function fixture() {
  ...
  await writeFile(join(source, 'ccg_coding_domain.py'), 'CODING_DOMAIN = 1\n')
...
describe('installReviewSupervisor', () => {
  it('installs private, executable scripts and a managed receipt', async () => {
    ...
    expect(receipt.files).toHaveProperty('ccg-coding-domain')
    expect(receipt.files).toHaveProperty('ccg_coding_domain.py')
    expect(receipt.files).toHaveProperty('ccg_model_compat.py')
    expect((await stat(join(install, 'bin', 'ccg-task'))).mode & 0o777).toBe(0o700)
    expect((await stat(join(install, 'bin', 'ccg-coding-domain'))).mode & 0o777).toBe(0o700)
  ...
  it('uninstalls only files still matching its receipt', async () => {
    ...
    expect(await uninstallReviewSupervisor(install)).toEqual(['ccg-agent-supervisor', 'ccg-task', 'ccg_task_router.py', 'ccg-coding-domain', 'ccg_coding_domain.py', 'ccg_model_compat.py', 'ccg_review_web.py', 'ccg_review_web.html', 'ccg_review_web.js', 'ccg_review_web.css', 'ccg_review_center_poster.webp'])
